...
define( 'DIRECCION_apg_sms_settings', plugin_basename( __FILE__ ) );
...
//Carga el idioma
load_plugin_textdomain( 'apg_sms', null, dirname( DIRECCION_apg_sms_settings ) . '/i18n/languages' );

//Enlaces adicionales personalizados
function apg_sms_enlaces( $enlaces, $archivo ) {
	global $apg_sms;

	if ( $archivo == DIRECCION_apg_sms_settings ) {
...
$plugin = DIRECCION_apg_sms_settings; 
...
function apg_sms_procesa_estados( $pedido, $notificacion = false ) {
	...
	if ( ( !$internacional || ( isset( $configuracion['internacional'] ) && $configuracion['internacional'] == 1 ) ) && !$notificacion ) {
		apg_sms_envia_sms( $configuracion, $telefono, $mensaje );
	}
}
...
function apg_sms_procesa_el_telefono( $pedido, $telefono, $servicio, $propietario = false ) {
	...
	if ( $servicio == "twilio" && strpos( $prefijo[1], "+" ) === false ) {
		$telefono = "+" . $telefono;
...
function apg_sms_muestra_mensaje() {
	...
	if ( !isset( $configuracion['mensaje_pedido'] ) || !isset( $configuracion['mensaje_nota'] ) ) {
		add_action( 'admin_notices', 'apg_sms_actualizacion' ); //Comprueba si hay que mostrar el mensaje de actualización
	}
}
