...
function apg_sms_envia_sms( $configuracion, $telefono, $mensaje ) {
	...
	switch ( $configuracion['servicio'] ) {
		...
		case "clockwork":
			...
			break;
		case "bulksms":
			$respuesta = wp_remote_post( "http://" . $configuracion['servidor_bulksms'] . "/eapi/submission/send_sms/2/2.0?username=" . urlencode( $configuracion['usuario_bulksms'] ) . "&password=" . urlencode( $configuracion['contrasena_bulksms'] ) . "&message=" . apg_sms_codifica_el_mensaje( $mensaje ) . "&msisdn=" . urlencode( $telefono ) );
			break;
		case "open_dnd":
